    ...
    <div class="grid grid-cols-3 gap-6">
        ...
        <div class="space-y-5">
            ...
            <div class="stat-card p-5 border border-red-100">
                ...
                @if($isOwner)
                    @if($members->count() > 1)
                        <p class="text-xs text-gray-500 mb-3">As the owner, you cannot close the colocation while there are other active members. You must remove them first.</p>
                        <button type="button" disabled class="w-full py-2 px-4 bg-gray-100 text-gray-400 text-sm font-600 rounded-xl border border-gray-200 cursor-not-allowed transition-colors">
                            Close Colocation (Disabled)
                        </button>
                    @else
                        <p class="text-xs text-gray-500 mb-3">Since you are the only remaining member, you can close this colocation permanently.</p>
                        <form action="{{ route('colocations.leave', $colocation->id) }}" method="POST" onsubmit="return confirm('Are you absolutely sure you want to close this colocation? This action is irreversible.');">
                            @csrf
